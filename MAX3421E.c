...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write val via SPI
	int return_code;
	BYTE data_to_be_written[2];
	data_to_be_written[0] = reg+2;
	data_to_be_written[1] = val;
	//read return code from SPI peripheral (see Intel documentation) 
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 2, (const alt_u8*) &data_to_be_written, 0, 0, 0);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int return_code;
	BYTE data_to_be_written[1+nbytes];
	data_to_be_written[0] = reg+2;
	for(int i = 1; i<nbytes+1; i++){
		data_to_be_written[i] = data[i-1];
	}
	//read return code from SPI peripheral (see Intel documentation) 
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes+1, (const alt_u8*) &data_to_be_written, 0, 0, 0);
	//if return code < 0  print an error
	if(return_code < 0){
			alt_printf("Error Occurred! \n");
	}
